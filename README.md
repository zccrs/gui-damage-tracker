...
|---|---|---|---|---|---|---|---|---|
| **空闲帧 (无脏位快速路径)** | 50 | 1 VP | **1.43 µs** | 699,011 FPS | 0.009% | 0.021% | 0.034% | 0.051% |
| **空闲帧 (无脏位快速路径)** | 200 | 1 VP | **1.45 µs** | 689,321 FPS | 0.009% | 0.021% | 0.035% | 0.052% |
| **空闲帧 (无脏位快速路径)** | 1000 | 2 VP | **2.11 µs** | 474,337 FPS | 0.013% | 0.030% | 0.051% | 0.076% |
| **空闲帧 (无脏位快速路径)** | 5000 | 4 VP | **3.43 µs** | 291,127 FPS | 0.021% | 0.049% | 0.082% | 0.123% |
| **局部内容损伤 (16x16 px)** | 50 (常规控件) | 1 VP | **68.54 µs** | 14,591 FPS | 0.411% | 0.987% | 1.645% | 2.465% |
| **局部内容损伤 (16x16 px)** | 50 (常规控件) | 2 VP (双屏) | **92.88 µs** | 10,766 FPS | 0.557% | 1.337% | 2.229% | 3.341% |
| **局部内容损伤 (16x16 px)** | 200 (复杂桌面) | 1 VP | **316.66 µs** | 3,158 FPS | 1.900% | 4.560% | 7.600% | 11.391% |
| **局部内容损伤 (16x16 px)** | 200 (复杂桌面) | 2 VP (双屏) | **422.88 µs** | 2,365 FPS | 2.537% | 6.089% | 10.149% | 15.212% |
| **局部内容损伤 (16x16 px)** | 1000 (合成器全树) | 1 VP | **852.29 µs** | 1,173 FPS | 5.114% | 12.273% | 20.455% | 30.658% |
| **单节点几何移动 (平移)** | 50 | 1 VP | **83.39 µs** | 11,992 FPS | 0.500% | 1.201% | 2.001% | 3.000% |
| **单节点几何移动 (平移)** | 200 | 1 VP | **381.17 µs** | 2,624 FPS | 2.287% | 5.489% | 9.148% | 13.711% |
| **子树层级旋转 (Transform)** | 200 | 1 VP | **227.72 µs** | 4,391 FPS | 1.366% | 3.279% | 5.465% | 8.191% |
| **背景采样扩散 (Backdrop)** | 200 | 1 VP | **325.98 µs** | 3,068 FPS | 1.956% | 4.694% | 7.823% | 11.726% |
| **高密度多节点损坏 (10% 脏)**| 200 | 1 VP | **341.71 µs** | 2,926 FPS | 2.050% | 4.921% | 8.201% | 12.292% |
...
+------------------+------------------+---------------------+-----------------------+
| 60 Hz            | 16.67 ms         | 0.41% (~0.07 ms)    | 1.90% (~0.32 ms)      |
| 120 Hz           | 8.33 ms          | 0.82% (~0.07 ms)    | 3.80% (~0.32 ms)      |
| 144 Hz           | 6.94 ms          | 0.99% (~0.07 ms)    | 4.56% (~0.32 ms)      |
| 240 Hz (电竞级)  | 4.17 ms          | 1.65% (~0.07 ms)    | 7.60% (~0.32 ms)      |
| 360 Hz (极限高刷)| 2.78 ms          | 2.47% (~0.07 ms)    | 11.39% (~0.32 ms)     |
+------------------+------------------+---------------------+-----------------------+
```

- **极速空闲检测**：当界面无元素变化时（如静态展示），Tracker 自动触发 `O(1)` 级别的干净状态快速短路，耗时仅 **1.4 微秒**，几乎零 CPU 开销。
- **高刷新率从容应对**：在 144Hz ~ 240Hz 极限高刷场景下，常规桌面界面（200 个图形节点）的损伤计算与遮挡剔除仅消耗不到 **7.6%** 的帧时间，在 1000 节点大型合成器下也能在 **0.85 ms** 内完成。
---
