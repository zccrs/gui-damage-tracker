...
Node *DemoScene::findNodeRecursive(Node *n, quint64 id) const
{
    if (!n)
        return nullptr;
    if (n->id() == id)
    ...
    for (Node *c = n->firstChild(); c; c = c->nextSibling()) {
        if (Node *f = findNodeRecursive(c, id))
            return f;
    }
    return nullptr;
}

Node *DemoScene::findNode(quint64 id) const
{
    return findNodeRecursive(m_root.get(), id);
}

Node *DemoScene::parentForInsert() const
{
    if (Node *sel = findNode(m_selectedId))
        return sel;
    return m_root.get();
}

bool DemoScene::isDescendantOf(const Node *node, const Node *ancestor) const
{
...
void DemoScene::moveNode(quint64 nodeId, quint64 newParentId, quint64 beforeSiblingId)
{
    ...
    Node *newParent = newParentId ? findNode(newParentId) : m_root.get();
    Node *before = beforeSiblingId ? findNode(beforeSiblingId) : nullptr;
...
void DemoScene::setRectSelected(qreal x, qreal y, qreal w, qreal h)
{
    ...
    if (!n || !n->toGeometry())
        return;
    n->toGeometry()->setBoundingRect(QRectF(x, y, w, h));
    maybeCommit();
}
...
void DemoScene::setExpansionSelected(int px)
{
    ...
    if (!n || !n->toBackdrop())
        return;
    n->toBackdrop()->setBackdropExpansion(px);
    maybeCommit();
}
...
void DemoScene::setClipExpansionSelected(bool clip)
{
    ...
    if (!n || !n->toBackdrop())
        return;
    n->toBackdrop()->setClipExpansion(clip);
    maybeCommit();
}
...
void DemoScene::buildDemoScene(const QString &name)
{
    ...
    if (name == QLatin1String("backdrop")) {
        ...
        backdrop->setBackdropExpansion(18);
...
void DemoScene::collectVisualOnly(Node *n, QVector<QVariantMap> *visual, int *paintOrder)
{
    if (n->isDisplayable()) {
        ...
        v.insert(QStringLiteral("occluded"), n->isFullyOccluded());
        v.insert(QStringLiteral("culled"), n->isCulled());
                              ...
                              int depth, int *paintOrder)
{
    ...
    } else if (n->type() == Node::Type::Backdrop) {
    ...
    row.insert(QStringLiteral("occluded"), n->isFullyOccluded());
    row.insert(QStringLiteral("culled"), n->isCulled());
...
void DemoScene::refreshSelectedProps()
{
    ...
    p.insert(QStringLiteral("isBackdrop"), n->toBackdrop() != nullptr);
    ...
    if (auto *bg = n->toBackdrop()) {
        p.insert(QStringLiteral("expansion"), bg->backdropExpansion().left());
        p.insert(QStringLiteral("clipExpansion"), bg->clipExpansion());
    }
    m_selectedProps = p;
    emit selectedPropsChanged();
}
